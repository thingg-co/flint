...
class Config:
    ...
    train_in_session: bool = _env("TRAIN_IN_SESSION", "0") not in ("0", "false", "no")   # let the idle-training loop run while the market is open too (a step can delay a forecast by one step, ~2 s on the Spark)
